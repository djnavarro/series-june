...
art_generator <- function(seed) {
  ...
  hex_noise <- runif(n, min = 1.25, max = 1.75)
  hex_scale <- runif(n, min = .5, max = 2.5)

  #bg <- "#000000"
  bg <- hex_shade[n + 1]

  row_order <- sample(1:n_row)
  col_order <- sample(1:n_col)
  ...
  for(row_ind in 1:n_row) {
    for(col_ind in 1:n_col) {
      ...
      if (runif(1) < 1 | is_corner) { 
        ...
        dplyr::mutate(
          fill = hex_shade[i], 
          x = x * hex_scale[i] + (c - 1) * 2 - (n_col - 1),
          y = y * hex_scale[i] + (r - 1) * 2 - (n_row - 1),
